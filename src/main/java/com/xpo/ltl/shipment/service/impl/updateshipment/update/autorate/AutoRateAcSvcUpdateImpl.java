...
@RequestScoped
public class AutoRateAcSvcUpdateImpl extends AbstractAcSvc implements AcSvcUpdate {
	...
	public Map<ActionCd, List<ShmAcSvc>> getItemsForTransactions(
		...
		List<ShmAcSvc> shmAcSvcListToDelete = this.getAbcShmAcSvcListToDelete(
		...
		List<ShmAcSvc> shmAcSvcListResetSeqNumber = this.resetSeqNumberShmAcSvcList(shipmentInstId,
	...
	private List<ShmAcSvc> resetSeqNumberShmAcSvcList(
			Long shipmentInstId,
		...
		List<ShmAcSvc> notDeletedUpdated = new ArrayList<>();
			...
			if (CollectionUtils.isNotEmpty(shmAcSvcsToUpdate)) {
				LoadValuesToUpdate<ShmAcSvc, ShmAcSvc> loadValuesToUpdate = new LoadValuesToUpdateManRateAcSvcImpl();
				notDeletedUpdated.addAll(loadValuesToUpdate.load(notDeleted, shmAcSvcsToUpdate, fromDelete, userId, AUTORATE_TRAN_CD));
