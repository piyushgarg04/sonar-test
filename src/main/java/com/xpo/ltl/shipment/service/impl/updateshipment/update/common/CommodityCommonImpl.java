...
@RequestScoped
public class CommodityCommonImpl extends AbstractCommodity implements CommodityTransactions {
	...
	public CompletableFuture<Map<ActionCd, List<ShmCommodity>>> cfGetAllTx(
		...
		return CompletableFuture.supplyAsync(() -> {
			try {
				return getAllTx(shmCommodities, shipment, updateShipmentRqst, userId, transactionCd,shipmentUpdateActionCd);
			} catch (ServiceException e) {
				getException(ServiceErrorMessage.COMMODITY_UPDATE_FAILED,
						this.getClass().getSimpleName(),
						e,
	...
	public List<ShmCommodity> getShmCommoditiesToUpdate(
		...
		updateShipmentRqst.getCommodities().forEach(commodity -> {
			...
			if (shmCommodityOptional.isPresent()) {
				...
				LoadValues<Commodity, ShmCommodity> commodityLoadValues = loadValuesFactory.getFactoryImplementation(
						ShmCommodity.class);

				commodityLoadValues.loadtValues(commodity, shmCommodity);
